...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        // Broze Challenge: Clearing Lines
        UITapGestureRecognizer *doubleTapRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(clearAll)];
...
-(void)drawRect:(CGRect)rect
{
    ...
    CGPoint velocityPoint = [moveRecognizer velocityInView:self];
    CGFloat currentVelocity = hypotf(velocityPoint.x, velocityPoint.y);
    ...
    if (velocity == 0) {
        ...
        ratio = currentVelocity / velocity;
        NSLog(@"%.2f / %.2f = %.2f",currentVelocity, velocity, ratio);
    }
...
- (void)moveLine:(UIPanGestureRecognizer *)gr
{
    ...
    // Silver Challenge: Mysterious Line - check if the selected line is in delete mode, don't move it
