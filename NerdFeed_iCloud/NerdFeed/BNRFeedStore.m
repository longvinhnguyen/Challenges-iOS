...
//        NSURL *dbURL = [NSURL fileURLWithPath:dbpath];
        ...
        NSURL *noSynDir = [ubContainer URLByAppendingPathComponent:@"feeb.nosync"];
        [fm createDirectoryAtURL:noSynDir withIntermediateDirectories:YES attributes:nil error:nil];
        
        // Specify the new file to the store Core Data's SQL Lite
        NSURL *dbURL = [noSynDir URLByAppendingPathComponent:@"feed.db"];
...
//        }
        ...
        if (![psc addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:dbURL options:options error:&error]) {
            [NSException raise:@"Reason: " format:@"%@",[error localizedDescription]];
        }
